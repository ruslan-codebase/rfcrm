...
async def get_company_by_id(company_id: UUID, session: AsyncSession = Depends(db.get_session)):
    company = await CompanyService(session).get_by_id(company_id)

    if company is None:
        raise HTTPException(
            status_code = 404,
            detail = f"Company not found with given id"
        )
...
@router.post("/")
async def add_company(company_in: CompanyIn, session: AsyncSession = Depends(db.get_session)):
    company = await CompanyService(session).create(company_in)
    return {"data": company, "message": ""}


@router.put("/{company_id}")
async def update_company(company_id: UUID, company_update: CompanyUpdate, session: AsyncSession = Depends(db.get_session)):
...
async def delete_company(company_id: UUID, session: AsyncSession = Depends(db.get_session)):
